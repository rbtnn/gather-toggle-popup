...
function! qficonv#encoding#iconv_utf8(line, else_encoding) abort
	let enc_from = ''
	if qficonv#encoding#contains_multichar(a:line)
		if qficonv#encoding#is_utf8(a:line)
			let enc_from = 'utf-8'
		else
			let enc_from = a:else_encoding
		endif
	endif
	if !empty(enc_from) && (enc_from != &encoding)
		return iconv(a:line, enc_from, &encoding)
	else
		return a:line
	endif
endfunction
